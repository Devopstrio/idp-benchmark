...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    ...
    { name: 'Security Benchmarks', path: '/security', icon: ShieldCheck },
    { name: 'Performance Analysis', path: '/performance', icon: Zap },
    { name: 'Cost Modeler', path: '/cost', icon: DollarSign },
    { name: 'Zero Trust Readiness', path: '/zero-trust', icon: Fingerprint },
    { name: 'Federation Handshake', path: '/federation', icon: Globe },
    { name: 'API Throughput', path: '/api-performance', icon: Cpu },
    { name: 'Directory Sync', path: '/directory', icon: Database },
    { name: 'Maturity Scoring', path: '/maturity', icon: TrendingUp },
    { name: 'Vendor Reports', path: '/reports', icon: FileText },
  ];
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          <div className="w-8 h-8 bg-teal-500 rounded-lg flex items-center justify-center shadow-lg shadow-teal-500/20">
            <BarChart3 className="text-slate-900 w-5 h-5" />
          </div>
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">IdPBench</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 truncate">Benchmarking Lead</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search benchmarks, vendors, metrics, reports..." 
                className="w-full bg-slate-800/50 border border-slate-700 rounded-lg py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all"
              />
            </div>
          </div>
